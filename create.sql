...
CREATE TABLE TB_MYSTOCK (
   USER_NO INT,
   FOREIGN KEY (USER_NO) REFERENCES TB_USER(USER_NO),
   STOCK_NAME VARCHAR(200) UNIQUE NOT NULL,
...
--24-01-30 모의투자 테이블 추가

ALTER TABLE `jolbu`.`tb_mock` 
ADD COLUMN `MOCK_LAST_PRICE` INT NULL DEFAULT 0 AFTER `SELL_MOCK_DATE`,
ADD COLUMN `MOCK_SELL_AMOUNT` INT NULL DEFAULT 0 AFTER `MOCK_LAST_PRICE`;